...
//	pid_init(&pid_pitch_motor_loc, 18000, 20, 2.2, 0, 10);
 // pid_init(&pid_pitch_motor_spd, 20000, 20, 5, 0, 0);
	...
	pid_init(&pid_pitch_motor_loc, 8700, 20, 2, 0.1, 50);
	 pid_init(&pid_pitch_motor_spd, 10000, 20, 1, 0, 1);
	...
	while (1)
	{
		spd = pid_calc(&pid_pitch_motor_loc, pitch_motor.total_angle, (rc.ch2/CH_MAX*90*36*8));
